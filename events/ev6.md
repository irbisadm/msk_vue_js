---
{
    "date":"19.12.2019",
    "timestamp":1576773000,
    "time":"19:30 — 22:00",
    "kind":"meetup",
    "number":"#6",
    "place":"Офис Ingram Micro Cloud",
    "regLink":"https://voximplant.timepad.ru/event/1139233/",
    "callForPaper":"https://docs.google.com/forms/d/e/1FAIpQLSdjvYW4xu9MT-8VRIIPTF5FSzq0ePEQGrH28h0z_xUtcdI9ew/viewform",
    "timeline":[
        {
            "time":"19:30 — 20:00",
            "name":"Сбор гостей, регистрация",
        },
        {
            "time":"20:00 — 20:30",
            "header":"Спикер уточняется - Ingram micro cloud",
            "name":"Доклад уточняется"
        },
        {
            "time":"20:30 — 21:00",
            "header":"Евгений Гаврилов - Ведущий разработчик в SuperJob",
            "name":"Используем конечные автоматы вместе с Composition API",
            "description":"Я расскажу о том, что такое конечные автоматы и покажу на примере, как можно использовать конечные автоматы вместе с  Composition API, а так же какие проблемы это поможет решить."
        },
        {
            "time":"21:00 — 21:30",
            "header":"Игорь Шеко – лид фронтов, Voximplant",
            "name": "Доклад уточняется"
        },
        {
            "time":"21:30 — 22:00",
            "name":"Свободное общение.",
        },
    ],
    "speakers": [
        {
            "avatar":"/speakers/sheko.jpg",
            "name":"Игорь Шеко",
            "company":"Lead frond-end, Voximplant",
            "bio":"До присоединения к команде Voximplant был сооснователем и техническим директором студии веб-разработки Appatbox и сервиса обратных звонков Rocketcallback. В настоящий момент разрабатывает WebSDK в Voximplant, участвует в социальных проектах для благотворительного фонда «Такие дела» и заботится о небольшой колонии пластиковых двухголовых динозавров. В составе команд разработки номинирован на несколько премий за Digital Storytelling в 2017 году (Visa pour l'image, Great Eight, Press Photo Digital Storytelling Contest) "
        },
        {
            "avatar":"/speakers/gavrilov.jpg",
            "name":"Евгений Гаврилов",
            "company":"Ведущий разработчик в SuperJob",
            "bio":"Ведущий разработчик в Superjob. С Vue.js активно контактирую с 2017 в основном в домашних проектах. Активный участник сообщества Vue - пишу статьи, временами контрибьютю vue-devtools. Обожаю эксперименты с кодом - богатый опыт фулстека позволяет делать с фронтендом то, что на первый взгляд свойственно только бэкенду."
        },
    ],
    "partners": [
        {
            "img":"/img/ingrammicro.png",
            "link":"https://www.ingrammicro.com/",
            "name":"Ingram Micro Cloud"
        },
        {
            "img":"/img/tproger_transparent_big.png",
            "link":"https://tproger.ru/",
            "name":"Tproger"
        },
        {
            "img":"/img/lovefrontend.jpg",
            "link":"https://vk.com/love_frontend",
            "name":"Love Frontend"
        },
        {
            "img":"/img/itmeeting.jpg",
            "link":"https://itmeeting.ru",
            "name":"IT Meeting"
        },
        {
            "img":"/img/eventsmoscow.jpg",
            "link":"https://t.me/EventsMoscow",
            "name":"Events Moscow | startupdigest"
        }
    ],
    "map":"https://yandex.ru/map-widget/v1/-/CGd2n8oE",
    "addr":"Офис Ingram Micro Cloud (Ленинградский просп., 15, стр. 18).",
}
---